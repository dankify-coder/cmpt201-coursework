...
Using write() to send data to the server. Using TCP protocol.
2. What data is the client sending to the server?
The client sends the strings Hello, Apple, Car, Green and Dog.
Understanding the Server:
1. Explain the argument that the `run_acceptor` thread is passed as an argument.
It receives a pointer to an acceptor_args struct which includes run, the list and the mutex.
2. How are received messages stored?
Stored in dynamically allocated memory, in a linked list.
3. What does `main()` do with the received messages?
Calls collect_all() to print and free every message.
4. How are threads used in this sample code?
Each thread is used to connect to a client.
*/

/*
Explain the use of non-blocking sockets in this lab.
How are sockets made non-blocking?
They are made non-blocking by using the O_NONBLOCK flag in the set_non_blocking function.
What sockets are made non-blocking?
The server and client sockets are all made non-blocking.
Why are these sockets made non-blocking? What purpose does it serve?
They are made non-blocking to prevent waiting for data and can return immediately.
*/
